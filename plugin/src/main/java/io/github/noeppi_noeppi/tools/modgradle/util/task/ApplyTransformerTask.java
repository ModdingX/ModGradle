...
public abstract class ApplyTransformerTask extends JarExec {
    ...
    public ApplyTransformerTask() {
        this.getTool().set(ModGradle.SOURCE_TRANSFORM);
        this.getArgs().addAll("transform", "--inheritance", "{inheritance}", "--mappings", "{mappings}", "--transformer", "{transformer}", "--remap", "--output", "{output}");
